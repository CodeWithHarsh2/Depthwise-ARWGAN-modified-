...
def train(model: ARWGAN,
    ...
    logging.info(f"Training images   : {len(train_data.dataset)}")
    logging.info(f"Validation images : {len(val_data.dataset)}")
    logging.info(f"Device            : {device}")
    logging.info(f"Epochs            : {train_options.number_of_epochs}")
    logging.info("=" * 60)
    ...
    for epoch in range(train_options.start_epoch, train_options.number_of_epochs + 1):
        ...
        logging.info('-' * 40)
        utils.save_checkpoint(model, train_options.experiment_name, epoch, "checkpoints")
